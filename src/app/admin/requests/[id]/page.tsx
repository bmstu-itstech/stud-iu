...
const valueClasses =
  "w-full px-6 py-5 sm:px-10 sm:py-8 bg-gray-50 rounded-2xl sm:rounded-[2.5rem] font-bold text-lg sm:text-3xl text-gray-900 border-2 border-transparent leading-normal break-words";
const labelClasses =
  "block text-sm sm:text-lg font-extrabold text-gray-400 uppercase tracking-widest ml-3 sm:ml-6 mb-2 sm:mb-4";
...
export default function RequestDetailsPage() {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await apiClient.put(`/join_requests/${id}`, {
        status,
      ...
      <div className="flex flex-col 2xl:flex-row justify-between items-start 2xl:items-center gap-6 sm:gap-8 mb-8 sm:mb-16">
        <div>
          <div className="flex flex-wrap items-center gap-3 sm:gap-6 mb-2 sm:mb-4">
            <Title className="!text-3xl sm:!text-6xl md:!text-7xl tracking-tight break-all">
              {req.name}
            </Title>
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 w-full 2xl:w-auto">
          <button
            onClick={() => router.back()}
            className="flex-1 2xl:flex-none px-8 py-4 sm:px-12 sm:py-6 rounded-2xl sm:rounded-[2.5rem] font-bold text-lg sm:text-2xl text-gray-500 bg-white border-2 border-gray-100 hover:bg-gray-50 transition-colors"
          >
            Назад
          </button>
          <button
            onClick={handleDelete}
            className="flex-1 2xl:flex-none px-8 py-4 sm:px-12 sm:py-6 rounded-2xl sm:rounded-[2.5rem] font-bold text-lg sm:text-2xl text-red-500 bg-red-50 hover:bg-red-100 transition-colors"
          >
            Удалить
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-12 gap-6 sm:gap-12 items-start">
        <div className="2xl:col-span-8 flex flex-col gap-6 sm:gap-12">
          <div className="bg-white p-6 sm:p-12 rounded-[2rem] sm:rounded-[3.5rem] shadow-sm border border-gray-100">
            <h3 className="text-gray-400 font-extrabold uppercase tracking-widest mb-6 sm:mb-10 text-base sm:text-xl ml-2">
              Контакты
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
              ...
              <a
                href={`https://t.me/${req.telegram.replace("@", "")}`}
                target="_blank"
                className="block group cursor-pointer hover:opacity-90 transition-opacity"
              >
                <span className={`${labelClasses} text-blue-400`}>
                  Telegram
                </span>
              ...
              {req.vk_link && (
                <a
                  href={req.vk_link}
                  target="_blank"
                  className="block col-span-1 md:col-span-2 group cursor-pointer hover:opacity-90 transition-opacity"
                >
                  <span className={`${labelClasses} text-blue-400`}>
                    VK Link
                  </span>
                  <div
                    className={`${valueClasses} !bg-blue-50 !text-blue-700 truncate`}
                  >
                    {req.vk_link}
                  </div>
                </a>
          ...
          <div className="bg-white p-6 sm:p-12 rounded-[2rem] sm:rounded-[3.5rem] shadow-sm border border-gray-100">
            <h3 className="text-gray-400 font-extrabold uppercase tracking-widest mb-6 sm:mb-12 text-base sm:text-xl ml-2">
              Ответы на вопросы
            </h3>
                ...
                currentFields.map((key) => {
                  ...
                  return (
                    <div key={key}>
                      ...
                      <div
                        className={`${valueClasses} min-h-[80px] sm:min-h-[100px] h-auto whitespace-pre-wrap`}
                      >
                        {Array.isArray(value) ? (
                          <div className="flex flex-wrap gap-2 sm:gap-4">
                            {value.map((tag: string) => (
                              <span
                                key={tag}
                                className="bg-white border-2 border-gray-200 px-4 py-2 sm:px-6 sm:py-3 rounded-xl sm:rounded-[1.5rem] text-sm sm:text-2xl font-bold text-gray-700 shadow-sm"
                              >
                                {tag}
                              </span>
                            ))}
        ...
        <div className="2xl:col-span-4 space-y-6 sm:space-y-12 sticky top-12">
              ...
              {STATUS_OPTIONS.map((opt) => (
                ...
                >
                  <div className="flex items-center gap-4 sm:gap-6">
                    <div
                      className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl sm:rounded-[2rem] flex items-center justify-center ${
          ...
          <div className="bg-white p-6 sm:p-12 rounded-[2rem] sm:rounded-[3.5rem] shadow-sm border border-gray-100">
            <h3 className="text-gray-400 font-extrabold uppercase tracking-widest mb-6 sm:mb-8 text-base sm:text-xl ml-2">
              Заметка
            </h3>
            <textarea
              value={comment}
