...
export default function EditEventPage() {
  ...
  const onSubmit = async (data: EventForm) => {
    ...
    formData.append(
      "start_datetime",
      parseToBackendFormat(data.start_datetime, data.precision),
    );
    if (data.end_datetime) {
      formData.append(
        "end_datetime",
        parseToBackendFormat(data.end_datetime, data.precision),
      );
    }
  ...
  const { onChange: onStartChange, ...restStart } = register("start_datetime", {
  ...
  return (
    <div className="flex flex-col gap-8 h-auto lg:h-[calc(100vh-6rem)]">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <Title className="!text-3xl sm:!text-5xl tracking-tight">
      ...
      <div className="flex flex-col lg:flex-row flex-1 gap-10 overflow-visible lg:overflow-hidden">
        <div className="w-full lg:w-1/2 bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 shadow-sm flex flex-col gap-6 overflow-y-auto border border-gray-200 custom-scrollbar h-auto lg:h-full">
          <div className="flex flex-col sm:flex-row gap-4 p-2 bg-gray-100 rounded-2xl">
            <label
              className={`flex-1 text-center py-3 sm:py-4 rounded-xl cursor-pointer transition-all font-bold text-lg ${wType === "FUTURE" ? "bg-white shadow-md text-blue-600" : "text-gray-500 hover:text-gray-700"}`}
            >
              <input
                type="radio"
                value="FUTURE"
            ...
            <label
              className={`flex-1 text-center py-3 sm:py-4 rounded-xl cursor-pointer transition-all font-bold text-lg ${wType === "PAST" ? "bg-white shadow-md text-blue-600" : "text-gray-500 hover:text-gray-700"}`}
            >
              <input
                type="radio"
                value="PAST"
          ...
          <input
            ...
            className="p-6 bg-gray-50 rounded-2xl text-xl font-bold placeholder:font-normal focus:ring-2 focus:ring-blue-500/20 outline-none transition-all"
            placeholder="Название события"
          />
          <textarea
            {...register("description")}
            className="p-6 bg-gray-50 rounded-2xl text-lg min-h-[140px] focus:ring-2 focus:ring-blue-500/20 outline-none transition-all resize-none"
          ...
          <div className="flex flex-col gap-4 bg-gray-50 p-6 rounded-2xl">
            <div className="flex items-center gap-4">
              ...
              <select
                {...register("precision")}
                className="flex-1 p-3 bg-white border border-gray-200 rounded-xl font-medium outline-none"
              >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              ...
              <input
                ...
                inputMode="numeric"
                placeholder={
                  wPrecision === "year" ? "Конец: ГГГГ" : "Конец (опционально)"
                }
                maxLength={getMaxLength(wPrecision)}
                {...restEnd}
                onChange={(e) => {
          ...
          <div className="flex flex-col sm:flex-row gap-6">
            ...
            <div className="relative w-full sm:w-24 h-16 sm:h-auto">
              <input
                type="color"
                {...register("color")}
                className="w-full h-full p-2 bg-gray-50 rounded-2xl cursor-pointer"
              />
            </div>
          </div>
          ...
          {wType === "FUTURE" ? (
            <input
              ...
              className="p-6 bg-blue-50/50 text-blue-800 rounded-2xl text-lg font-medium outline-none border border-blue-100 placeholder:text-blue-300"
              placeholder="Ссылка на регистрацию"
            />
          ) : (
            <input
              {...register("album_link")}
              className="p-6 bg-green-50/50 text-green-800 rounded-2xl text-lg font-medium outline-none border border-green-100 placeholder:text-green-300"
              placeholder="Ссылка на фотоальбом"
            />
          )}

          <div className="mt-auto pt-4">
            <label className="block text-gray-400 font-bold mb-2 uppercase text-xs tracking-wider">
              Изменить обложку
            </label>
            <input
              type="file"
        ...
        <div className="hidden xl:flex w-1/2 bg-gray-100 rounded-[2.5rem] p-10 items-center justify-center relative border border-dashed border-gray-300">
          <div className="absolute top-8 right-8 bg-white px-4 py-2 rounded-full text-xs font-bold text-gray-400 tracking-widest uppercase shadow-sm">
            Live Preview
          </div>
          ...
          <div className="scale-110 origin-center pointer-events-none">
            <EventCard
              ...
              start_datetime={
                wDate
                  ? parseToBackendFormat(wDate, wPrecision)
